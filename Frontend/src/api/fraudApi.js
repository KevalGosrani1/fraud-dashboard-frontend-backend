...
export const riskApi = {
  ...
  getWalletRisk: async (walletAddress) => {
    const response = await apiClient.get(`/api/risk/${walletAddress}`);
    return response.data;
  },
...
export const walletApi = {
  ...
  flagWallet: async (walletAddress) => {
    const response = await apiClient.post('/api/flag', { walletAddress });
    return response.data;
  },

  // Get on-chain report count
  getReportCount: async (walletAddress) => {
    const response = await apiClient.get(`/api/report-count/${walletAddress}`);
    return response.data;
  },
